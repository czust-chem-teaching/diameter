...
# ============================================================
# Paired RCC certificate ablation
# ============================================================

def run_certificate_ablation(file_name):
    """
    Controlled ablation of the SCC-DAG escape certificate.

    Common work is performed once:
      * graph loading and deduplication
      * SCC decomposition and condensation-DAG construction
      * the same 500 forward/reverse sentinel BFS searches
      * the same RS_k local-expansion scan

    During the common sentinel pass, two rigorous upper-bound arrays are built:
      ub_same : same-SCC certification only
      ub_full : same-SCC certification plus the SCC-DAG escape certificate

    Stage 3 exact verification is executed only for the full RCC method in
    order to obtain the exact diameter D.

    The two certificate systems are then compared under the same exact lower
    bound LB = D. This isolates certificate strength without running a separate
    and potentially much larger exact-verification workload for the same-SCC
    baseline.

    Important:
      "Residual at D" is not an observed Stage-3 BFS count for the same-SCC
      baseline. It is the number of nodes that remain unresolved after the
      common early-termination filter when the lower bound is fixed at D.
    """
    ...
    print("\n" + "=" * 90)
    print(f"RCC ESCAPE-CERTIFICATE ABLATION: {file_name}")
    print("=" * 90)
    ...
    # Algorithm timing below excludes input-file parsing.
    ...
    # --------------------------------------------------------
    # Stage 1: one common sentinel sample builds both upper-bound arrays
    # --------------------------------------------------------
    print("\nStage 1: one common 500-sentinel pass builds both certificate systems")
    ...
    # Stream sentinel results to avoid storing all reverse-distance arrays simultaneously.
    with Pool(
        ...
        for sentinel, ssspl, ssspn, ecc_s, d_to_s in pool.imap(
            ...
            # Same-SCC-only certified upper bound.
            ub_same[safe_same] = np.minimum(
                ub_same[safe_same],
                score[safe_same]
            )

            # Full RCC includes the same-SCC certificate.
            ub_full[safe_same] = np.minimum(
                ub_full[safe_same],
                score[safe_same]
            )

            # SCC-DAG escape certificate.
    ...
    print(f"Same-SCC coverage = {same_coverage}/{num_nodes} "
    ...
    # --------------------------------------------------------
    # Stage 2: one common local-expansion scan
    # --------------------------------------------------------
    ...
    print(f"Same-SCC candidates at LB0 = {same_candidates} "
    ...
    # --------------------------------------------------------
    # Stage 3: run exact verification only for full RCC to obtain D
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Certificate-strength comparison at the common exact diameter D
    # --------------------------------------------------------
    # With LB fixed at the exact diameter D, any active node with
    # UB <= D is certified by the corresponding certificate. Nodes with
    # UB > D remain unresolved by that certificate. These residual counts
    # are certification measures, not observed Stage-3 BFS counts for the
    # same-SCC baseline.
    ...
    print("\n" + "=" * 90)
    print("RCC ESCAPE-CERTIFICATE ABLATION RESULT")
    print("=" * 90)
    print(f"Exact diameter D                         : {exact_diameter}")
    print(f"Same-SCC coverage                  : {same_coverage / num_nodes * 100:.2f}%")
    ...
    print(f"Same-SCC residual unresolved at D      : {residual_same}")
    ...
    return {
        ...
        "Same-SCC Coverage": same_coverage,
        "Same-SCC Coverage (%)": round(same_coverage / num_nodes * 100.0, 2),
        ...
        "Same-SCC Candidates at LB0": same_candidates,
        ...
        "Same-SCC Residual at D": residual_same,
...
if __name__ == "__main__":
    ...
    for net in NETWORK_FILES:
        res = run_certificate_ablation(net)
        if res is not None:
            all_results.append(res)

            pd.DataFrame(all_results).to_csv(
                "rcc_escape_ablation_results_backup.csv",
                index=False
            )
    ...
    if all_results:
        ...
        print("\n" + "=" * 220)
        print("FINAL RCC ESCAPE-CERTIFICATE ABLATION RESULTS")
        print("=" * 220)
        print(final_df.to_string(index=False))
        print("=" * 220)

        output_file = "rcc_escape_ablation_results.csv"
